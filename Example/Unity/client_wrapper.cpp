...
const char* GetToken(ClientWrapper* wrapper)
{
    return wrapper->client.GetToken().Token.c_str();
}
...
// InitClient
void ClientWrapper_InitClient(ClientWrapper* wrapper, const char* ClientId, const char* ClientVer,
                              ConnectionStateCallbackType ConnectionStateCallback, PacketCallbackType PacketCallback,
                              LogCallbackType LogCallback, SessionTokenCallbackType SessionTokenCallback)
{
    ...
    wrapper->client.InitClient(ClientId, ClientVer,
                               ...
                               [PacketCallback](std::shared_ptr<Inworld::Packet> Packet)
                               {
                                   if (Packet != nullptr)
                                   {
                                       ...
                                       // If the packet has a data chunk, encode it in Base64.
                                       if (pkt.has_data_chunk())
                                       {
                                           std::string encodedData = Base64Encode(
                                               reinterpret_cast<const unsigned char*>(pkt.data_chunk().chunk().data()),
                                               pkt.data_chunk().chunk().size());
                                           pkt.mutable_data_chunk()->set_chunk(encodedData);
                                       }

                                       // Convert the packet to its JSON representation.
                                       std::string stringPacket = SerializeProtoToJson(pkt);
                                       const int paddingSize = 8; // or 10 or whatever you deem appropriate
                                       stringPacket += std::string(paddingSize, '@');
                                       int length = stringPacket.size();

                                       // Ensure the JSON length is valid for int representation.
                                       if (length > (size_t)INT_MAX)
                                       {
                                           Inworld::Log("Error: Serialized JSON length exceeds maximum int value");
                                           return;
                                       }

                                       // Allocate memory for the JSON data.
                                       unsigned char* allocatedMemory = static_cast<unsigned char*>(CoTaskMemAlloc(
                                           length));
                                       if (!allocatedMemory)
                                       {
                                           Inworld::Log("Error: Failed to allocate memory for serialized JSON.");
                                           return;
                                       }

                                       // Copy the JSON data to the allocated memory.
                                       std::memcpy(allocatedMemory, stringPacket.c_str(), length);

                                       // Log and send the data to the callback.
                                       Inworld::Log(
                                           "ABOUT TO JSON SERIALIZE and send PACKET IN NDK JSON length IS " +
                                           std::to_string(
                                               length));
                                       PacketCallback(allocatedMemory, length, GetPacketType(pkt));
                                   }
                               });
    ...
    Inworld::Log("INITIALIZED CLIENT FROM DLL");
}

void ClientWrapper_GenerateToken(ClientWrapper* wrapper, SessionTokenCallbackType SessionTokenCallback)
{
    auto info = wrapper->client.GetToken();
    wrapper->client.GenerateToken([SessionTokenCallback, info]()
    {
        std::string jsonString = SerializeSessionInfo(info);
...
// StartClient
void ClientWrapper_StartClientWithCallback(ClientWrapper* wrapper, const uint8_t* serialized_options,
                                           int serialized_options_size, const uint8_t* serialized_sessionInfo,
                                           int serialized_sessionInfo_size, LoadSceneCallbackType LoadSceneCallback)
{
...
void ClientWrapper_CancelResponse(ClientWrapper* wrapper, const char* AgentId, const char* InteractionId,
                                  const char** UtteranceIds, int UtteranceIdsCount)
{
...
void ClientWrapper_SendStringSoundMessage(ClientWrapper* wrapper, const char* AgentId, const char* Data)
{
    std::string audioData(Data);
    wrapper->client.SendSoundMessage(AgentId, audioData);
}

// SendSoundMessageWithAEC
void ClientWrapper_SendSoundMessageWithAEC(ClientWrapper* wrapper, const char* AgentId, const int16_t* InputData,
                                           int InputDataSize, const int16_t* OutputData, int OutputDataSize)
{
...
// GetConnectionError
bool ClientWrapper_GetConnectionError(ClientWrapper* wrapper, char* OutErrorMessage, int BufferSize,
