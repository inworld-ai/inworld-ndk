...
std::shared_ptr<Inworld::DataEvent> Inworld::ClientBase::SendTextureMessage(const std::string& AgentId, const std::string& Data)
{
	auto Packet = std::make_shared<TextureDataEvent>(Data, Routing::Player2Agent(AgentId));
	SendPacket(Packet);
	return Packet;
}
